...
public class ShortcutsControlPanel extends LinearLayout {
    ...
    private void createIcon() {
        ...
        for (int i = 0; i < mDataList.size() && mDataList.size() <= AMOUNT_OF_ICONS; i++) {
    ...
    private void fillInDataListIcon() {
        ...
        List<ResolveInfo> pkgAppsList = mContext.getPackageManager().queryIntentActivities(
                mainIntent, 0);
            ...
            for (DataHolder dh : mDataList) {
                if (info.activityInfo.packageName.equals(dh.pkgName)) {
                    if (info.activityInfo.name.equals(dh.clzName)) {
                        dh.appIconDrawable = info.loadIcon(mContext.getPackageManager());
                    }
                }
            }
        }
